...
export class VersionsTab {
	...
	/** Assert the version at `index` offers the given action. */
	async expectActionPresent(index: number, action: VersionAction): Promise<void> {
		const menu = await this.openMenu(index)
		await expect(this.actionItem(menu, action)).toBeVisible()
		await this.closeMenu(index)
	}
}
