...
test.describe('Settings: App management', () => {
	test.beforeEach(async ({ page, appstorePage }) => {
		...
		await page.request.post('/ocs/v2.php/apps/appstore/api/v1/apps/disable', {
			data: { appId: 'testing' },
		}).catch(() => {})

		// Enable update notification app if disabled
		await page.request.post('/ocs/v2.php/apps/appstore/api/v1/apps/enable', {
			data: { appId: 'updatenotification' },
		}).catch(() => {})
	...
	test('View app details', async ({ appstorePage }) => {
		...
		await expect(appstorePage.enableButton('QA testing')).toBeVisible()
	...
	test('Limit app usage to group', async ({ page, appstorePage }) => {
		// Open the "Active apps" section
		await appstorePage.openEnabledApps()

		// Select the Dashboard app
		await appstorePage.appLink('Dashboard').click({ force: true, timeout: 10000 })
		...
		await expect(appstorePage.limitedToGroupsList()).not.toBeVisible()
	})
})
