...
trait S3ObjectTrait {
	...
	public function supportsConditionalWrites(): bool {
		...
		// 'auto' (default): probe the bucket once and cache the outcome. A null result
		// is a transient probe failure — treat it as unsupported for this request but
		// do not memoize it, so a later request can retry the probe.
		$probed = $this->probeConditionalWrites();
		if ($probed === null) {
			return false;
		}
		return $this->conditionalWritesSupported = $probed;
	}
