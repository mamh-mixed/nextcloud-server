...
abstract class AShareRecipientTypeSearchCollaborator implements IShareRecipientTypeSearch {
	...
	public function searchRecipients(ShareAccessContext $accessContext, string $query, int $limit, int $offset): array {
		...
		if (Server::get(IUserSession::class)->getUser()?->getUID() !== $accessContext->currentUser->getUID()) {
		...
		$searchResults = Server::get(ISearch::class)->search($query, [$this->getCollaboratorType()], false, $limit, $offset);
		if ($searchResults === []) {
			return [];
		}
