...
class EventsSearchProviderTest extends TestCase {
	...
	// Recurring event whose occurrences all fall before the ranges used in
	// the tests below, so it resolves to no in-range occurrence and is
	// dropped by resolveComponent().
	private static string $vEvent9 = 'BEGIN:VCALENDAR' . PHP_EOL
		. 'VERSION:2.0' . PHP_EOL
		. 'PRODID:-//Tests//' . PHP_EOL
		. 'CALSCALE:GREGORIAN' . PHP_EOL
		. 'BEGIN:VEVENT' . PHP_EOL
		. 'UID:out-of-range@example.com' . PHP_EOL
		. 'DTSTAMP:20000101T080000Z' . PHP_EOL
		. 'DTSTART;TZID=Europe/Berlin:20000101T090000' . PHP_EOL
		. 'DTEND;TZID=Europe/Berlin:20000101T100000' . PHP_EOL
		. 'RRULE:FREQ=YEARLY;COUNT=3' . PHP_EOL
		. 'SUMMARY:Out of range event' . PHP_EOL
		. 'END:VEVENT' . PHP_EOL
		. 'END:VCALENDAR';
...
ICS;
	...
	public function testSearchDropsInvalidRowsWithoutStallingCursor(): void {
		...
		// page whenever every result on it gets dropped.
		$user = $this->createMock(IUser::class);
		$user->method('getUID')->willReturn('john.doe');
		$query = $this->createMock(ISearchQuery::class);
		$query->method('getFilter')->willReturnCallback(function ($name) {
		...
		$this->backend->expects($this->once())
			...
			->willReturn([
				[
					'calendarid' => 99,
					'calendartype' => CalDavBackend::CALENDAR_TYPE_CALENDAR,
					'uri' => 'out-of-range-event.ics',
					'calendardata' => self::$vEvent9,
				],
				[
					'calendarid' => 99,
					'calendartype' => CalDavBackend::CALENDAR_TYPE_CALENDAR,
		...
		$this->urlGenerator->method('linkTo')->willReturn('link-to-remote.php');
		$this->urlGenerator->method('linkToRoute')->willReturn('link-to-route-calendar/');
		$this->urlGenerator->method('getAbsoluteURL')->willReturn('deep-link-to-calendar');

		$actual = $this->provider->search($user, $query);
		$data = $actual->jsonSerialize();

		// Both backend rows are consumed even though one was dropped, so the
		// cursor advances past both instead of stalling on the dropped row.
		$this->assertCount(1, $data['entries']);
		$this->assertEquals('Recurring yearly event', $data['entries'][0]->jsonSerialize()['title']);
		$this->assertEquals(22, $data['cursor']);
	}

	public function testResolveComponentReturnsNullForMalformedData(): void {
		// Reader::read() can throw on data that isn't merely a non-calendar
		// document (see testResolveComponentReturnsNullForNonCalendar) but is
		// outright unparsable. That must be treated as a dropped row too,
		// instead of failing the whole search request.
		$since = new \DateTimeImmutable('2024-06-02T00:00:00Z');
		$until = new \DateTimeImmutable('2024-06-03T00:00:00Z');

		$actual = self::invokePrivate($this->provider, 'resolveComponent', ['this is not calendar data', $since, $until]);
